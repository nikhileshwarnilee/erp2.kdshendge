...
class Homework_model extends MY_model
{
    ...
    public function searchdailyassignment($class_id, $section_id, $subject_group_id, $subject_group_subject_id, $date)
    {
        ...
        $this->datatables->select('daily_assignment.*,staff.name,staff.surname,staff.employee_id,classes.class,sections.section,students.firstname,students.middlename,students.lastname,students.id as student_id,students.admission_no as student_admission_no,subjects.name as subject_name,subjects.code as subject_code')
            ...
            ->orderable('students.firstname,classes.class,sections.section,subjects.name,daily_assignment.description,daily_assignment.remark,daily_assignment.date,daily_assignment.evaluation_date')
            ->sort('students.firstname,classes.class,sections.section,subjects.name,daily_assignment.title,daily_assignment.date,daily_assignment.description', 'DESC')
            ->from('daily_assignment');
        return $this->datatables->generate('json');
    }   
    ...
    public function search_dthomeworkreport($class_id, $section_id, $subject_group_id, $subject_id)
    {
        ...
        foreach ($matrix as $class_id => $sections) {
            foreach ($sections as $section_id) {
                $allowed[(int)$class_id][(int)$section_id] = true;
            }
        }

        return array_values(array_filter($result, function ($row) use ($allowed) {
            $class_id   = isset($row['class_id']) ? (int)$row['class_id'] : (isset($row['class']) ? (int)$row['class'] : 0);
    ...
    //===============
      public function search_homework_marks_report($class_id, $section_id, $subject_group_id, $subject_id)
    {
        if ((!empty($class_id)) && (!empty($section_id)) && (!empty($subject_id)) && (!empty($subject_group_id))) {
            $this->db->where(array('homework.class_id' => $class_id, 'homework.section_id' => $section_id, 'subject_groups.id' => $subject_group_id, 'subject_group_subjects.id' => $subject_id));
        } 
        ...
        $this->db->select("`homework`.*,classes.class,sections.section,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name as subject_name,subjects.code as subject_code,subject_groups.id as subject_groups_id,subject_groups.name,
            students.admission_no,
            students.roll_no,
            students.firstname,
            students.middlename,
            students.lastname,`homework_evaluation`.`marks` as marks_obtain,`homework_evaluation`.`note`,`homework_evaluation`.`date` as eval_date ");
        
        $this->db->join("homework_evaluation", "homework.id = homework_evaluation.homework_id");
        $this->db->join("student_session", "student_session.id = homework_evaluation.student_session_id");
        $this->db->join("students", "students.id = student_session.student_id");

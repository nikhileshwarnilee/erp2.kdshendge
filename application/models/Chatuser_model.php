...
class Chatuser_model extends CI_Model
{
    ...
    public function searchForUser($keyword, $chat_user_id, $login_id, $user_type = 'staff')
    {
        if ($user_type == 'staff') {
            ...
            $sql = "SELECT staff.id AS staff_id,NULL AS student_id,staff.name,staff.surname,staff.image FROM staff WHERE staff.name LIKE '%$keyword%'
                    AND staff.is_active = 1 AND staff.id NOT IN (SELECT a.staff_id FROM chat_users a WHERE a.staff_id IN (SELECT cu2.staff_id
                    FROM chat_users cu1
                    INNER JOIN chat_connections cc ON cc.chat_user_one = cu1.id
                    INNER JOIN chat_users cu2 ON cu2.id = cc.chat_user_two
                    WHERE cu1.student_id = $login_id
                    AND cu1.user_type = 'student'))";
        } else if ($user_type == 'parent') { //added
    ...
    {
        if ($user_type == 'staff') {
            $sql = " SELECT * from chat_connections WHERE chat_connections.chat_user_one= (SELECT id FROM `chat_users` WHERE staff_id=" . $staff_id . "  and user_type='staff' ) or chat_connections.chat_user_two = (SELECT id FROM `chat_users` WHERE staff_id=" . $staff_id . "   and user_type='staff') ORDER BY `chat_connections`.`id` DESC";
        } else if ($user_type == 'student') {
            $sql = " SELECT * from chat_connections WHERE chat_connections.chat_user_one= (SELECT id FROM `chat_users` WHERE student_id=" . $staff_id . "   and user_type='student') or chat_connections.chat_user_two = (SELECT id FROM `chat_users` WHERE student_id=" . $staff_id . "   and user_type='student') ORDER BY `chat_connections`.`id` DESC";
        } else if ($user_type == 'parent') { //added
            $sql = " SELECT * from chat_connections WHERE chat_connections.chat_user_one= (SELECT id FROM `chat_users` WHERE student_id=" . $staff_id . "   and user_type='parent') or chat_connections.chat_user_two = (SELECT id FROM `chat_users` WHERE student_id=" . $staff_id . " and
                    user_type='parent') ORDER BY `chat_connections`.`id` DESC";
        }
    ...
    {
        $sql = "SELECT chat_users.id as `chat_user_id`, chat_users.user_type,chat_users.student_id,staff_id ,students.guardian_pic,students.guardian_name,students.father_name,students.firstname,students.middlename,students.lastname,staff.name,staff.surname,(CASE WHEN staff_id IS NULL THEN (students.gender) ELSE (staff.gender) END) as gender,(CASE WHEN staff_id IS NULL THEN (SELECT
students.image as `image` FROM students WHERE students.id=chat_users.student_id) ELSE (SELECT staff.image as `image` FROM staff WHERE staff.id=chat_users.staff_id) END) as image FROM `chat_users` left JOIN students on students.id=chat_users.student_id left JOIN staff on staff.id=chat_users.staff_id WHERE chat_users.id=" . $chat_user_id;
    ...
    {
        $sql           = "SELECT id FROM `chat_messages` WHERE chat_connection_id=" . $chat_connection_id;
        $query         = $this->db->query($sql);
        $chat_messages = $query->result();
        return $chat_messages;
    }

    public function get_chat_msg_count($staff_id)
    {
        $sql  = "select id from chat_messages where `chat_user_id`=(SELECT id FROM `chat_users` WHERE staff_id=$staff_id and user_type='staff' ) and `is_read`=0 group by `chat_connection_id`";
        $query         = $this->db->query($sql);
        $chat_messages = $query->result();
        return $chat_messages;
    }
    ...
    {
        if ($user_type == 'student') {
            $sql = " SELECT id from chat_messages WHERE chat_messages.chat_user_id= (SELECT id FROM `chat_users` WHERE student_id=" . $student_id . "   and user_type='student') and `is_read`=0  group by `chat_connection_id`";
        } else if ($user_type == 'parent') {
            $sql = " SELECT id from chat_messages WHERE chat_messages.chat_user_id= (SELECT id FROM `chat_users` WHERE student_id=" . $student_id . "   and user_type='parent')  and `is_read`=0   group by `chat_connection_id`";
        }
        $query         = $this->db->query($sql);
        $chat_messages = $query->result();
        return $chat_messages;

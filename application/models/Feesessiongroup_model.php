...
class Feesessiongroup_model extends MY_Model
{
    ...
    public function getFeesByGroup($id = null, $display_system = NULL, $only_search_due_fees = 0){
        ...
        foreach ($result as $key => $value) {
            $value->feetypes = $this->getfeeTypeByGroup($value->id, $value->fee_groups_id, $only_search_due_fees);
            if ($only_search_due_fees && empty($value->feetypes)) {
                continue;
            }
	...
	public function getfeeTypeByGroup($fee_session_group_id, $id = null, $only_search_due_fees = 0)
    {
        $this->db->select('fee_groups_feetype.*,feetype.type,feetype.code,feetype.include_in_search_due_fees');
        ...
        if ($only_search_due_fees) {
            $this->db->where('feetype.include_in_search_due_fees', 1);
        }

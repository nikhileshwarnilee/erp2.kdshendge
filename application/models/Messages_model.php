...
class Messages_model extends MY_Model
{
    ...
    public function delete_email_template($id)
    {
        ...
        if (!empty($attachment_list)) {
            foreach ($attachment_list as $key => $attachment_list_value) {
                ...
                $delete_file_size = $this->media_storage->getUploadedFileSize($attachment_list_value['attachment'], 'uploads/communicate/email_template_images/');
			    $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);

                $this->db->where('attachment', $attachment_list_value['attachment']);
    ...
    public function schedule($id = null,$type = null)
    {
        $this->db->select()->from('messages');
		if($type == 'schedule'){
    ...
    public function get_email_attachment_list($message_id)
    {
        $this->db->select('email_attachments.*');
        $this->db->from('email_attachments');
        $this->db->where('email_attachments.message_id', $message_id);
    ...
    {
        $this->db->where('is_schedule != ', 1);
        $this->db->where('sent', 1);
        $this->db->delete('messages');
    }

     public function getEmailTemplateAttachmentByTemplateID($template_id)
    {
        $this->db->select('email_template_attachment.*');
        $this->db->from('email_template_attachment');
        $this->db->where('email_template_attachment.email_template_id', $template_id);

...
class Mailer
{
    ...
    public function __construct()
    {
        ...
        $this->email_header = $this->CI->setting_model->get_email_header();
        $this->email_footer = $this->CI->setting_model->get_email_footer();
    }

    public function send_email_view($body, $subject)
    {
        // Load a view and pass data to it
        $data['body'] = $body;
        $data['subject'] = $subject;
        $data['email_header'] = $this->email_header;
        $data['email_footer'] = $this->email_footer;

        // Load the view
        $html_content = $this->CI->load->view('admin/notification/_emailtemplatedesign', $data, TRUE);

        // Do something with the $html_content, like sending an email
        return $html_content;
    }
    ...
    public function send_mail_marksheet($toemail, $subject, $body, $file, $file_name, $cc = "")
    {
        ...
        if ($this->CI->mail_config->email_type == "aws_ses") {
            ...
            if ($this->CI->mail_config->email_type == "smtp") {
                ...
                $mail->SetFrom($this->CI->mail_config->smtp_email, $school_name);
                $mail->AddReplyTo($this->CI->mail_config->smtp_email, $school_name);
            } else {
        ...
        if ($this->CI->mail_config->email_type == "aws_ses") {
            ...
            if (!empty($FILES) && isset($FILES['files'])) {
                ...
                if (is_array($FILES['files']['name'])) {
                    ...
                    for ($i = 0; $i < $no_files; $i++) {
                        if (
                            !empty($FILES['files']['tmp_name'][$i]) &&
                            is_uploaded_file($FILES['files']['tmp_name'][$i])
                        ) {
                ...
                // Single file
                    ...
                    if (
                        !empty($FILES['files']['tmp_name']) &&
                        is_uploaded_file($FILES['files']['tmp_name'])
                    ) {

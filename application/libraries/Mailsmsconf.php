...
class Mailsmsconf
{
    ...
    public function mailsms($send_for, $sender_details, $date = null, $exam_schedule_array = null, $file = null)
    {
        ...
        if (!empty($chk_mail_sms)) {
            if ($send_for == "student_admission") {
                ...
                if ($chk_mail_sms['template'] != "" && $chk_mail_sms['whatsapp']) {
					$this->CI->load->library('whatsappgateway');
                    if (!empty($contact_numbers)) {
                        foreach ($contact_numbers as $key => $contactvalue) {
                            $this->CI->whatsappgateway->sentRegisterWhatsapp($sender_details['student_id'], $contactvalue, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);                       
                        }
                    }
                }
                ...
                $this->sendResult($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'], $chk_mail_sms['whatsapp_template_id']);
				...
				if ($chk_mail_sms['template'] != "" && $chk_mail_sms['whatsapp']) {
					$this->CI->load->library('whatsappgateway'); 
                    foreach ($contact_numbers as $key => $contact_numbersvalue) {
                        $this->CI->whatsappgateway->sentAddGroupFeeWhatsapp($sender_details, $contact_numbersvalue, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);                       
                ...
                if ($chk_mail_sms['template'] != "" && $chk_mail_sms['whatsapp']) {
					$this->CI->load->library('whatsappgateway'); 
                    foreach ($contact_numbers as $key => $contact_numbersvalue) {
                        $this->CI->whatsappgateway->sentAddFeeWhatsapp($sender_details, $contact_numbersvalue, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);                       
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($sms_detail) && $chk_mail_sms['student_recipient']) { 
					$this->CI->load->library('whatsappgateway');
                    $send_to=$sender_details->mobileno;
                    $this->CI->whatsappgateway->sentFeeProcessingNotification($sender_details,$send_to,$chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                    
                }				
		 
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($sms_detail) && $chk_mail_sms['guardian_recipient']) { 
					$this->CI->load->library('whatsappgateway');
                    $send_to=$sender_details->guardian_phone;
                    $this->CI->whatsappgateway->sentFeeProcessingNotification($sender_details,$send_to,$chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                    
                }
                
            } elseif ($send_for == "student_absent_attendence") {
                $this->sendAbsentAttendance($chk_mail_sms, $sender_details, $date, $chk_mail_sms['template'], $exam_schedule_array, $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "") {
					$this->CI->load->library('whatsappgateway');
                    foreach($contact_numbers as $key => $contact_numbersvalue){
                        $send_to=$contact_numbersvalue;
                        $this->CI->whatsappgateway->sentfeesreminderNotification($sender_details,$send_to,$chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                    }
                }
                //==========whatsapp============//
                ...
                $this->sendHomework($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                
            }elseif ($send_for == "homework_evaluation") {

                $this->sendHomeworkevaluation($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" ) {
                    ...
                    foreach ($contact_numbers as $key => $contact_numbersvalue) {
                        $sender_details['contact_no']=$contact_numbersvalue;
						$this->CI->whatsappgateway->sendStudentLoginCredential($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
					}
                }
				...
				if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" ) {   
					$this->CI->load->library('whatsappgateway');
					$this->CI->whatsappgateway->sendStaffLoginCredential($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                }
                ...
                if($chk_mail_sms['staff_recipient']){
                    ...
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" ) {
                        ...
                        foreach ($stafflist as $key => $value) {
                            $sender_details['contact_no'] = $value['contact_no'];
                            $this->CI->whatsappgateway->student_apply_leave($sender_details, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                        }
                    }
                }
                ...
                $this->sendPresentAttendance($chk_mail_sms, $sender_details, $date, $chk_mail_sms['template'], $exam_schedule_array, $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
    ...
    public function sendResult($chk_mail_sms, $exam_result, $template, $subject, $template_id, $whatsapp_template_id)
    {
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($exam_result['exam_result'])) {
                foreach ($exam_result['exam_result'] as $res_key => $res_value) {
					 ...
					 //==================
                    $token = $this->encode_marksheet_token(
                        $exam_result['marksheet'],
                        $res_value->student_id,
                        $exam->id,
                        $exam->exam_group_id,
                        $res_value->exam_group_class_batch_exam_student_id
                    );

                    $marksheet_pdf_url = base_url()."download-exam-marksheet/".$token;
                    //==================										
                    $detail = array(
                    ...
                    if ($chk_mail_sms['whatsapp']  && $chk_mail_sms['template'] != "") {
						$this->CI->load->library('whatsappgateway');
                       $this->CI->whatsappgateway->sentExamResultWhatsapp($detail, $template, $whatsapp_template_id);
                    }
    ...
    //======================
    private function encode_marksheet_token($marksheet,$student_id,$exam_id,$exam_group_id,$exam_group_class_batch_exam_student_id) {
        $data = implode('|', [
            $marksheet,
            $student_id,
            $exam_id,
    ...
    public function sendPresentAttendance($chk_mail_sms, $student_session_array, $date, $template, $subject_attendence, $subject, $template_id,$whatsapp_template_id)
    {
            ...
            if (!empty($student_result)) {
                ...
                foreach ($student_result as $student_result_k => $student_result_v) {
                    ...
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" &&  $chk_mail_sms['student_recipient']) { 
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sendPresentAttendancenotification($detail,$template,$whatsapp_template_id,$detail['mobileno']);
                    }

                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" &&  $chk_mail_sms['guardian_recipient']) { 
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sendPresentAttendancenotification($detail,$template,$whatsapp_template_id,$detail['guardian_phone']);
                    }
                    // ------------------whatsapp-------------------//
                }
            }
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($student_result)) {
                foreach ($student_result as $student_result_k => $student_result_v) {
                    ...
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && $chk_mail_sms['staff_recipient']) {
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sentPresentStaffWhatsapp($detail,$template,$chk_mail_sms['whatsapp_template_id']);
                    }
                }
            }
        }
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification']  or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($student_result)) {
                foreach ($student_result as $student_result_k => $student_result_v) {
                    ...
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && $chk_mail_sms['staff_recipient']) {
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sentAbsentStaffWhatsapp($detail, $template,$chk_mail_sms['whatsapp_template_id']);
                    }
                }
            }
        }
    ...
    public function sendAbsentAttendance($chk_mail_sms,$student_session_array,$date,$template,$subject_attendence,$subject,$template_id,$whatsapp_template_id)
    {
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($student_result)) {
                ...
                foreach ($student_result as $student_result_k => $student_result_v) {
                    ...
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" &&  $chk_mail_sms['student_recipient']) { 
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sendAbsentAttendancenotification($detail,$template,$whatsapp_template_id,$detail['mobileno']);
                    }

                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" &&  $chk_mail_sms['guardian_recipient']) { 
						$this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sendAbsentAttendancenotification($detail,$template,$whatsapp_template_id,$detail['guardian_phone']);
                    }
                    // ------------------whatsapp-------------------//
                }
            }
    ...
    public function sendHomework($chk_mail_sms, $student_details, $template, $email_subject, $template_id,$whatsapp_template_id)
    {
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($student_list)) {
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "") { 
					$this->CI->load->library('whatsappgateway');
                    $this->CI->whatsappgateway->sendstudentlhomework($student_sms_list,$template,$whatsapp_template_id);
                }
                // ------------------whatsapp-------------------//
   ...
   public function sendHomeworkevaluation($chk_mail_sms, $student_details, $template, $email_subject, $template_id,$whatsapp_template_id){
                ...
                foreach($student_list as $student_key => $student_value) {
                    ...
                    if ($student_value['app_key'] != "" && $chk_mail_sms['student_recipient']){
                        $student_notification_list[] = array(
                            ...
                            'evaluation_date'   => date($this->CI->customlib->getSchoolDateFormat(), $this->CI->customlib->dateYYYYMMDDtoStrtotime($student_value['evaluation_date'])),
                        );
                    }
    ...
    public function sendOnlineexam($chk_mail_sms, $student_details, $template, $subject, $template_id)
    {
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification']  or $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($student_list)) {
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "") {  
					$this->CI->load->library('whatsappgateway');
                    $this->CI->whatsappgateway->sentOnlineexamStudentWhatsapp($student_sms_list, $template,$chk_mail_sms['whatsapp_template_id']);
                }
                // ------------------whatsapp-------------------//
    ...
    {        
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            //===============whatsapp=================//
            if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['student_recipient'] && $student_details['mobileno']) {
				$this->CI->load->library('whatsappgateway');
                $this->CI->whatsappgateway->sendOnlineadmissionformsubmit($student_details, $template,$student_details['mobileno'],$chk_mail_sms['whatsapp_template_id']);
            }
            if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['guardian_recipient'] && $student_details['guardian_phone']) {
				$this->CI->load->library('whatsappgateway');				
                $this->CI->whatsappgateway->sendOnlineadmissionformsubmit($student_details, $template,$student_details['guardian_phone'],$chk_mail_sms['whatsapp_template_id']);
            }
            //===============whatsapp=================//
        }
    }
    ...
    public function sendOnlineadmissionFees($chk_mail_sms, $student_details, $template, $subject, $template_id)
    {
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification'] or $chk_mail_sms['whatsapp']) {
            ...
            //=========whatsapp=============//
            if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['student_recipient']) {
				$this->CI->load->library('whatsappgateway');
                $this->CI->whatsappgateway->sentstudentOnlineadmissionFeessubmissionWhatsapp($student_details, $template,$student_details['mobileno'],$chk_mail_sms['whatsapp_template_id']);
            }
            if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['guardian_recipient']) { 
				$this->CI->load->library('whatsappgateway');
                $this->CI->whatsappgateway->sentstudentOnlineadmissionFeessubmissionWhatsapp($student_details, $template,$student_details['guardian_phone'],$chk_mail_sms['whatsapp_template_id']);
            }
            //=========whatsapp=============//

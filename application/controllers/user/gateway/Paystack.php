...
class Paystack extends Studentgateway_Controller
{
    ...
    public function paystack_pay()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data                           = array();
            $amount                         = (float) convertBaseAmountCurrencyFormat(($params['fine_amount_balance'] + $params['total'] - $params['applied_fee_discount']+ $params['gateway_processing_charge']));
             $total                          = $amount;
            $data['student_id']             = $params['student_id'];
           $data['total']                  = (int) round($total * 100);
                ...
                if ($result['data']) {
                    ...
                    if ($result['data']['status'] == 'success') {
                        ...
                        foreach ($params['student_fees_master_array'] as $fee_key => $fee_value) {
							...
							if(isset($fee_value['applied_fee_discount'])){
								$applied_fee_discount = $fee_value['applied_fee_discount'];
							}else{
								$applied_fee_discount = 0;
							}

                            $json_array = array(
                                'amount'          =>  $fee_value['amount_balance']-$applied_fee_discount,
                                'date'            => date('Y-m-d'),
                                'amount_discount' => $applied_fee_discount,
                        ...
                        $send_to     = $params['guardian_phone'];
							...
							if(isset($params['fee_discount_group'])){
								$fee_discount_group = $params['fee_discount_group'];
							}else{
								$fee_discount_group = null;
							}
						
						
                        $response = $this->studentfeemaster_model->fee_deposit_bulk($bulk_fees, $fee_discount_group);
                        //========================

        ...
        if ($this->form_validation->run() == false) {
         ...
         try {
            ...
            if(IsNullOrEmptyString($img_name)){  // check upload image has not uploaded successfully
                $total_documents_failed_size += $this->media_storage->getTmpFileSize('timeline_doc');  // get temp size of image because of image not uploaded 
            }
            ...
            $array = array('status' => 'success', 'error' => '', 'message' => $msg);
             ...
             $array = array('status' => 'success', 'error' =>  $e->getMessage(), 'message' => '');
        } 
        }  
    ...
    public function edit()
    {
        ...
        try {
            ...
            if (isset($_FILES["timeline_doc"]) && $_FILES['timeline_doc']['name'] != '' && (!empty($_FILES['timeline_doc']['name']))) {
                if (!empty($timeline_list['document'])) {
                    $prev_file_size = $this->media_storage->getUploadedFileSize($timeline_list['document'], 'uploads/student_timeline');
                }
                ...
                if (!IsNullOrEmptyString($img_name)) {
                    ...
                    $failed_size = $this->media_storage->getTmpFileSize('timeline_doc');
                    if ($failed_size > 0) {
                        $this->saasvalidation->deleteResouceQuota('storage', $failed_size);
                    }
                }
            } else {

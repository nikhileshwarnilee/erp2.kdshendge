...
class Homework extends Student_Controller
{
    ...
    public function createdailyassignment()
    {
        ...
        if ($this->form_validation->run() == false) {
			...
			try {
				...
				// ANY ERROR INSIDE TRY WILL COME HERE
				$array = array(
					'status' => 'fail',
        ...
        if ($this->form_validation->run() == false) {
			...
			try {
				...
				if (isset($_FILES["file"]) && $_FILES['file']['name'] != '' && (!empty($_FILES['file']['name']))) {
					...
					$prev_file_size = $this->media_storage->getUploadedFileSize($assignment_list['attachment'], '/uploads/homework/daily_assignment/');
    ...
    public function deletedailyassignment($id)
    {
        ...
        if ($row['attachment'] != '') {
			...
			$delete_file_size = $this->media_storage->getUploadedFileSize($row['attachment'], 'uploads/homework/daily_assignment/');

...
if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
    ...
    public function dtadmissionreport()
    {
        ...
        if (!empty($resultlist->data)) {
            foreach ($resultlist->data as $resultlist_key => $student) {
                ...
                $row[] = "<span class='pull-left'>".$student->admission_no."</span>";
                $row[] = $viewbtn;
            ...
            $footer_row[] = "";
            $footer_row[] = "";
            $footer_row[] = "<b>".$this->lang->line('total_admission_in_this_duration')."</b>";
            $footer_row[] = "<b>".$filter_label."</b>";
            $footer_row[] = "<b>".$count."</b>";
            $footer_row[] = "";
            $footer_row[] = "";
            $footer_row[] = "";
            $footer_row[] = "";
    ...
    public function dtexamreportlist()
    {
        ...
        $sch_setting = $this->sch_setting_detail;
        $results     = $this->onlineexam_model->dtonlineexamReport($condition);
    ...
    public function dtexamattemptreport()
    {
        ...
        if (!empty($resultlist->data)) {
            foreach ($resultlist->data as $resultlist_key => $student_value) {
                ...
                $row[] =  "<a  href='" . base_url() . "student/view/" . $student_value->id . "'>" .$this->customlib->getFullName($student_value->firstname, $student_value->middlename, $student_value->lastname, $sch_setting->middlename, $sch_setting->lastname)."</a>";
...
if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
    ...
    public function dtonlineadmissionreportlist()
    {
        ...
        if (!empty($resultlist->data)) {
            foreach ($resultlist->data as $resultlist_key => $online_admission) {
                ...
                $row[] = "<a href= '" . base_url() . "student/view/" . $online_admission->id . "'>". $this->customlib->getFullName($online_admission->firstname, $online_admission->middlename, $online_admission->lastname, $this->sch_setting_detail->middlename, $this->sch_setting_detail->lastname). "</a>";
    ...
    public function leaverequestreport()
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/human_resource');
        $this->session->set_userdata('subsub_menu', 'Reports/attendance/leaverequestreport');
		...
		$from_date_sql 		= !empty($from_date) ? date('Y-m-d', $this->customlib->datetostrtotime($from_date)) : null;
		$to_date_sql 		= !empty($to_date) ? date('Y-m-d', $this->customlib->datetostrtotime($to_date)) : null;
		$joining_date_sql 	= !empty($joining_date) ? date('Y-m-d', $this->customlib->datetostrtotime($joining_date)) : null;
		$condition 			= '';

		if ($from_date !='' && $to_date!="") {
            $condition .= " and date_format(staff_leave_request.leave_from,'%Y-%m-%d') >= '$from_date_sql' and  date_format(staff_leave_request.leave_from,'%Y-%m-%d') <='$to_date_sql' ";
        }

        if ($joining_date !='') {
            $condition .= " and date_format(staff.date_of_joining,'%Y-%m-%d') =  '$joining_date_sql' ";
        }

        if ($staff_name !='') {
            $condition .= " and staff_leave_request.staff_id =  $staff_name";
        }
		if ($leave_status !='') {
            $condition .= " and staff_leave_request.status =  ".strtolower("'$leave_status'")."";
        }
		
        $data["status"]        	= 	$this->status; 
        $data['resultlist'] 	= 	$this->leaverequest_model->getleaverequestreport($condition);
        $data['staff_list'] 	= 	$this->staff_model->searchFullText("", 1);        

        $this->load->view('layout/header', $data);
        $this->load->view('reports/leaverequestreport', $data); 
        $this->load->view('layout/footer', $data); 
    }


    public function myleaverequestreport()
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/human_resource');
        $this->session->set_userdata('subsub_menu', 'Reports/human_resource/myleaverequestreport');
        ...
        $data["status"]         =   $this->status; 
        $data['resultlist']     =   $this->leaverequest_model->getmyleaverequestreport($condition);
        $data['staff_list']     =   $this->staff_model->searchFullText("", 1);
        $data['staff_role']     =   $userdata['role_id'] ;

        $this->load->view('layout/header', $data);
        $this->load->view('reports/myleaverequestreport', $data);
        $this->load->view('layout/footer', $data);
    }

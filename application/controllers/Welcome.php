...
{ 
    ...
    public function admission()
    {
        if ($this->module_lib->hasActive('online_admission')) {
			...
			$storage_array = "document,father_pic,mother_pic,guardian_pic,file";         
            ...
            if ($this->form_validation->run() == false) {
				...
				try {
					...
					if ($document_validate) {
						...
						$fieldsstorage = ['file', 'father_pic', 'mother_pic', 'guardian_pic', 'document'];
						
						$this->saasvalidation->updateStorageLimit('storage', $fieldsstorage);
						...
						if (isset($_FILES["document"]) && !empty($_FILES['document']['name'])) {
							...
							if (IsNullOrEmptyString($img_name)) {
								...
								$total_image_failed_size += $this->media_storage->getTmpFileSize('document');								 
						...
						$upload_path = "./uploads/student_images/online_admission_image/";
						...
						if (!empty($_FILES['file']['name'])) {
								...
								if (IsNullOrEmptyString($img_name)) {
									...
									$total_image_failed_size += $this->media_storage->getTmpFileSize('file');
						...
						$fields = ['father_pic', 'mother_pic', 'guardian_pic'];
						
						
						foreach ($fields as $field) {
							if (!empty($_FILES[$field]['name'])) {
								$img_name = $this->media_storage->fileupload($field, $upload_path);									 

                                if (IsNullOrEmptyString($img_name)) {

                                    $total_image_failed_size += $this->media_storage->getTmpFileSize($field);
                                    continue;
                                }								
								
								$data[$field] = 'uploads/student_images/online_admission_image/' . $img_name;
							}
						}
    ...
    public function editonlineadmission($reference_no)
    {
        ...
        if ($ref_status) {
            ...
            if (!empty($this->input->post('admission_id'))) {
				...
				$storage_array = "document,father_pic,mother_pic,file,guardian_pic";
                    ...
                    $this->load_theme('pages/editadmission', $this->config->item('front_layout'));
					 ...
					 try{
						...
						if ($document_validate) {
							...
							if (isset($_FILES["document"]) && !empty($_FILES['document']['name'])) {								
								$prev_file_size += $this->media_storage->getUploadedFileSize($result['document'], 'uploads/student_documents/online_admission_doc');
								$img_name         = $this->media_storage->fileupload("document", "./uploads/student_documents/online_admission_doc/");
								
								if (!IsNullOrEmptyString($img_name)) {
	
									$total_image_upload_size += $this->media_storage->getTmpFileSize('document');
								}
							...
							$upload_path = "./uploads/student_images/online_admission_image/";
							$db_path     = "uploads/student_images/online_admission_image/";						 
							
							if (!empty($_FILES['file']['name'])) {							
								
								$prev_file_size += $this->media_storage->getUploadedFileSize($result['file'],$db_path);							
							...
							foreach ($fields as $field) {
								...
								if (!empty($_FILES[$field]['name'])) {							
									...
									$prev_file_size += $this->media_storage->getUploadedFileSize($result[$field],$db_path);							
								
									$img_name = $this->media_storage->fileupload($field, $upload_path);							
								
									if (!IsNullOrEmptyString($img_name)) {
										$total_image_upload_size += $this->media_storage->getTmpFileSize($field);
									}							
								
									$data[$field] = $db_path . $img_name;
								}
							}							

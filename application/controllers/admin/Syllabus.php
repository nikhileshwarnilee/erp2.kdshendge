...
class Syllabus extends Admin_Controller
{
    ...
    public function delete_subject_syllabus()
    {
        ...
        if ($syllabus_details['attachment'] != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($syllabus_details['attachment'], 'uploads/syllabus_attachment');
        ...
        if ($syllabus_details['lacture_video'] != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($syllabus_details['lacture_video'], 'uploads/syllabus_attachment/lacture_video');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);             
    ...
    public function add_syllabus()
    {
        ...
        $storage_array = "file,lacture_video"; // use comma for multiple files       
        ...
        try {
            ...
            $storage_array = ['file','lacture_video'];
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if (IsNullOrEmptyString($attachment_img_name)) {  // check upload image has not uploaded successfully
            ...
            if (isset($_FILES["lacture_video"]) && !empty($_FILES['lacture_video']['name'])) {
                ...
                if (IsNullOrEmptyString($lacture_video_img_name)) {  // check upload image has not uploaded successfully
                    $total_documents_failed_size += $this->media_storage->getTmpFileSize('lacture_video');  // get temp size of image because of image not uploaded 
                }
    ...
    public function handle_upload()
    {
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            if (empty($tmp_name) || !is_uploaded_file($tmp_name) || !file_exists($tmp_name)) {
                $this->form_validation->set_message('handle_upload', $this->lang->line('invalid_file_format_or_size'));
                return false;
            }

...
class Pickuppoint extends Admin_Controller
{
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('pickup_point', 'can_edit')) {                    
                    $action = '<button class="btn btn-primary btn-xs pickup_map" data-pick-location="' . $value->id . '"data-toggle="tooltip" title="' . $this->lang->line("map") . '"><i class="fa fa-map-marker"></i></button> <a onclick="edit(' . $value->id . ')" class="btn btn-primary btn-xs"  data-toggle="tooltip" title="' . $this->lang->line('edit') . '"><i class="fa fa-pencil"></i></a>';         
                }else{
                    $action = '';
                }
                
                if ($this->rbac->hasPrivilege('pickup_point', 'can_delete')) {
                    $deletebtn = " <a href=" . base_url() . 'admin/pickuppoint/delete_point/' . $value->id . " class='btn btn-primary btn-xs' data-toggle='tooltip'  title=" . $this->lang->line('delete') . " '  onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' ><i class='fa fa fa-remove'></i></a>";
                }else{
                    $deletebtn = '';
                }
    ...
    public function check_dublicate_pickuppoint($route_id,$pickup_point_id)
    {
        $total_count=$this->pickuppoint_model->is_duplicate($route_id,$pickup_point_id);
        if (!empty($total_count)) {
            $this->form_validation->set_message('check_dublicate_pickuppoint', $this->lang->line('duplicate_pickup_point_found'));		
            return false;
        }
    }
    ...
    public function create()
    {
        if ($_POST['action_type'] == 'add') {
                ...
                $pickup_point_id=implode(",",$_POST['pickup_point']);
                $route_id=$_POST['route_id'];

                $allValid = true;
                foreach ($_POST['pickup_point'] as $key => $value) {
                    if ($value === '' || $value === null) {
                        $allValid=false;
                        break;
                    }
                ...
                if(($allValid!=false) && !empty($route_id)){
                    $this->form_validation->set_rules(
                    'route_id',
                    'Route',
                    "callback_check_dublicate_pickuppoint[$pickup_point_id]"
                    );
                }
        } else {
                $this->form_validation->set_rules('route_id', $this->lang->line('route'), 'trim|required|xss_clean');

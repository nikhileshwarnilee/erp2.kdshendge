...
class Leaverequest extends Admin_Controller
{
    ...
    public function remove($id, $staff_id)
    {
        ...
        if ($row['document_file'] != ''){
            $delete_file_size = $this->media_storage->getUploadedFileSize($row['document_file'], "uploads/staff_documents/$staff_id");
    ...
    {
        $date_1 = date("Y-m-d", $this->customlib->datetostrtotime($this->input->post('date_1')));
        $date_2 = date("Y-m-d", $this->customlib->datetostrtotime($this->input->post('date_2')));
        $differenceFormat = '%a';
        ...
        if ($date_1 && $date_2) {
            ...
            $datetime2 = date_create($date_2);
            $interval  = date_diff($datetime1, $datetime2);
             echo $interval->format($differenceFormat) + 1;        
    ...
    public function addLeave()
    {
        ...
        $this->form_validation->set_rules('userfile', $this->lang->line('storage'), "callback_validateCanUploadFile[$storage_array]");      
        ...
        try {
            ...
            if($half_day_leave){
				if($leave_days > 1){
                    ...
                    $half_day_leave = 'first_half';
				}else{
					$leave_days = 0.5;
                    $half_day_leave = 'second_half';
				}
			}
            ...
            if ($total_remain >= $leave_days) {
                ...
                //=================*****===================
                    ...
                    //=================saas==============
                    if (IsNullOrEmptyString($document)) {  // check upload image has not uploaded successfully
            ...
            $total_remain = $my_laeve['alloted_leave'] - $my_laeve['total_applied'];
                ...
                if ($total_remain >= $leave_days) {
                    ...
                    if (!empty($request_id)) {
                        ...
                        $session = $this->setting_model->getCurrentSession();//current session id 
                        $data = array('staff_id' => $staff_id, 'date' => date("Y-m-d", $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document,'session_id'=>$session,'half_day_leave'    => $half_day_leave,);
                    }
                    ...
                    if(isset($this->sch_setting_detail->staff_notification_email) && $this->sch_setting_detail->staff_notification_email!=""){

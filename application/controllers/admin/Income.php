...
class Income extends Admin_Controller
{
    ...
    public function delete($id)
    {
        ...
        if ($row['documents'] != '') {
			$delete_file_size = $this->media_storage->getUploadedFileSize($row['documents'], 'uploads/school_income');
        ...
        if ($this->form_validation->run() == false) {
			...
			try {
				...
				if (isset($_FILES["documents"]) && $_FILES['documents']['name'] != '' && (!empty($_FILES['documents']['name']))) {
						...
						$prev_file_size = $this->media_storage->getUploadedFileSize($income['documents'], 'uploads/school_income');
						$img_name = $this->media_storage->fileupload("documents", "./uploads/school_income/");
	
						if (!IsNullOrEmptyString($img_name)) {
							$total_image_upload_size += $this->media_storage->getTmpFileSize('documents');
						}
    ...
    public function getincomelist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($value->documents) {
                    $documents = "<a href='" . base_url() . "admin/income/download/" . $value->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }

                if ($this->rbac->hasPrivilege('income', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/income/edit/" . $value->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }
				
                if ($this->rbac->hasPrivilege('income', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' href='" . base_url() . "admin/income/delete/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                } 

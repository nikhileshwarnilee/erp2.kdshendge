...
class Itemstock extends Admin_Controller
{
    ...
    public function index()
    {
		...
		$storage_array = "item_photo"; // use comma for multiple files       
        ...
        try {
            ...
            if (IsNullOrEmptyString($img_name)) {  // check upload image has not uploaded successfully
                $total_documents_failed_size += $this->media_storage->getTmpFileSize('item_photo');  
    ...
    public function delete($id)
    {
        ...
        if ($row['attachment'] != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row['attachment'], 'uploads/inventory_items');
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            if (isset($_FILES["item_photo"]) && $_FILES['item_photo']['name'] != '' && (!empty($_FILES['item_photo']['name']))) {
                $prev_file_size = $this->media_storage->getUploadedFileSize($item['attachment'], 'uploads/inventory_items');
                $img_name = $this->media_storage->fileupload("item_photo", "./uploads/inventory_items/");
                if (!IsNullOrEmptyString($img_name)) {
                    $total_image_upload_size += $this->media_storage->getTmpFileSize('item_photo');
                }

...
class Certificate extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('background_image', $this->lang->line('storage'), "callback_validateCanUploadFile[$storage_array]");      
        ...
        try {
                ...
                if (IsNullOrEmptyString($picture)) {  // check upload image has not uploaded successfully
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            if (!empty($_FILES['background_image']['name'])) {
                $prev_file_size = $this->media_storage->getUploadedFileSize($editcertificate[0]->background_image, 'uploads/certificate');
                ...
                if (!IsNullOrEmptyString($img_name)) {
                 ...
                 $data['background_image']= $editcertificate[0]->background_image;
            }
    ...
    public function delete($id)
    {
        ...
        if ($row[0]->background_image != '') {
            ...
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->background_image, 'uploads/certificate');

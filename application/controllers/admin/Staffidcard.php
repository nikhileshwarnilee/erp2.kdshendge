...
class Staffidcard extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $storage_array = "background_image,logo_img,sign_image"; // use comma for multiple files       
        ...
        try {
            ...
            $storage_array = ['background_image','logo_img','sign_image'];
            ...
            if (!empty($_FILES['background_image']['name'])) {
                ...
                if (IsNullOrEmptyString($background_img_name)) {  // check upload image has not uploaded successfully
                    $total_documents_failed_size += $this->media_storage->getTmpFileSize('background_image');  // get temp size of image because of image not uploaded 
                }
            ...
            if (!empty($_FILES['logo_img']['name'])) {
                ...
                if (IsNullOrEmptyString($logo_img_name)) {  // check upload image has not uploaded successfully
                    $total_documents_failed_size += $this->media_storage->getTmpFileSize('logo_img');  // get temp size of image because of image not uploaded 
                }
                if ($total_documents_failed_size > 0) {
                    $this->saasvalidation->deleteResouceQuota('storage', $total_documents_failed_size);
                }
            ...
            if (!empty($_FILES['sign_image']['name'])) {
                ...
                if (IsNullOrEmptyString($sign_img_name)) {  // check upload image has not uploaded successfully
                    $total_documents_failed_size += $this->media_storage->getTmpFileSize('sign_image');  // get temp size of image because of image not uploaded 
                }
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            if ($removebackground_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editstaffidcard[0]->background , 'uploads/staff_id_card/background');
            ...
            if ($removelogo_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editstaffidcard[0]->logo , 'uploads/staff_id_card/logo');
				$this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);         
            ...
            if ($removesign_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editstaffidcard[0]->sign_image , 'uploads/staff_id_card/signature');
				$this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);         
            ...
            if (isset($_FILES["background_image"]) && $_FILES['background_image']['name'] != '' && (!empty($_FILES['background_image']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editstaffidcard[0]->background, 'uploads/staff_id_card/background');
                $background         = $this->media_storage->fileupload("background_image", "./uploads/staff_id_card/background/");
                $data['background'] = $background;
                  if (!IsNullOrEmptyString($background)) {
            ...
            if (isset($_FILES["logo_img"]) && $_FILES['logo_img']['name'] != '' && (!empty($_FILES['logo_img']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editstaffidcard[0]->logo, 'uploads/staff_id_card/logo');

                $logo_img     = $this->media_storage->fileupload("logo_img", "./uploads/staff_id_card/logo/");
                $data['logo'] = $logo_img;
                if (!IsNullOrEmptyString($logo_img)) {
                    $total_image_upload_size += $this->media_storage->getTmpFileSize('logo_img');
                }
            }
            ...
            if (isset($_FILES["sign_image"]) && $_FILES['sign_image']['name'] != '' && (!empty($_FILES['sign_image']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editstaffidcard[0]->sign_image, 'uploads/staff_id_card/signature');

                $sign_image         = $this->media_storage->fileupload("sign_image", "./uploads/staff_id_card/signature/");
                $data['sign_image'] = $sign_image;
                if (!IsNullOrEmptyString($sign_image)) {
                    $total_image_upload_size += $this->media_storage->getTmpFileSize('sign_image');
                }
            }
    ...
    public function delete($id)
    {
        ...
        if ($row[0]->background != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->background, 'uploads/staff_id_card/background');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);   
            $this->media_storage->filedelete($row[0]->background, "uploads/staff_id_card/background/");
        }

        if ($row[0]->logo != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->logo , 'uploads/staff_id_card/logo');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size); 
            $this->media_storage->filedelete($row[0]->logo, "uploads/staff_id_card/logo/");
        }

        if ($row[0]->sign_image != '') {
              $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->sign_image , 'uploads/staff_id_card/signature');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);         

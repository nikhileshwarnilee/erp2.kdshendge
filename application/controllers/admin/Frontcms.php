...
class Frontcms extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $storage_array = 'logo,fav_icon'; // use comma for multiple files       
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["logo"]) && !empty($_FILES["logo"]['name'])) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($row->logo, 'uploads/school_content/logo');
               
                $img_name = $this->media_storage->fileupload("logo", "./uploads/school_content/logo/");

                if (!IsNullOrEmptyString($img_name)) {
                    $total_image_upload_size += $this->media_storage->getTmpFileSize('logo');
                }
            ...
            if (isset($_FILES["fav_icon"]) && !empty($_FILES["fav_icon"]['name'])) {
                ...
                if (!IsNullOrEmptyString($img_name)) {
                    $total_image_upload_size += $this->media_storage->getTmpFileSize('fav_icon');
                }

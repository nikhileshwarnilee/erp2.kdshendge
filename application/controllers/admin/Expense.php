...
class Expense extends Admin_Controller
{
    ...
    public function delete($id)
    {
        ...
        if ($row['documents'] != '') {
			$delete_file_size = $this->media_storage->getUploadedFileSize($row['documents'], 'uploads/school_expense');
        ...
        if ($this->form_validation->run() == false) {
			...
			try {
				...
				if (isset($_FILES["documents"]) && $_FILES['documents']['name'] != '' && (!empty($_FILES['documents']['name']))) {
					$prev_file_size = $this->media_storage->getUploadedFileSize($expense['documents'], 'uploads/school_expense');
					$img_name = $this->media_storage->fileupload("documents", "./uploads/school_expense/");
    ...
    public function getexpenselist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('expense', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/expense/edit/" . $value->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }
                if ($this->rbac->hasPrivilege('expense', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/expense/delete/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                }

                if ($value->documents) {
                    $documents = "<a href='" . base_url() . "admin/expense/download/" . $value->id . "' class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'>

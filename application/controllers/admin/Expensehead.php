        ...
        if (!empty($expense_head->data)) {           
            ...
            foreach ($expense_head->data as $exhead_key => $exhead_value) {
                ...
                if ($this->rbac->hasPrivilege('expense_head', 'can_edit')) {
                    $action .= "<a href='" . site_url('admin/expensehead/edit/' . $exhead_value->id) . "' class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }
                if ($this->rbac->hasPrivilege('expense_head', 'can_delete')) {
                    $action .= "<a href='" . site_url('admin/expensehead/delete/' . $exhead_value->id) . "' class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('delete') . "' onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");'><i class='fa fa-remove'></i></a>";
                }
                $action.="</div>";
                $row           = array();

...
class Admitcard extends Admin_Controller
{
    ...
    public function edit($id)
    {
        ...
        $this->session->set_userdata('sub_menu', 'Examinations/admitcard');
        $this->data['admitcardList'] = $this->admitcard_model->get();
        $this->data['admitcard'] = $admitcard_data = $this->admitcard_model->get($id);
        ...
        try {
            ...
            if(isset($_FILES["left_logo"]) && $_FILES['left_logo']['name'] != '' && (!empty($_FILES['left_logo']['name']))) {
                ...
                $prev_file_size += $this->media_storage->getUploadedFileSize($this->data['admitcard']->left_logo,'uploads/admit_card');
                ...
                $insert_data['left_logo'] = $this->data['admitcard']->left_logo;
            }
            ...
            if(isset($_FILES["right_logo"]) && $_FILES['right_logo']['name'] != '' && (!empty($_FILES['right_logo']['name']))) {
                ...
                $prev_file_size += $this->media_storage->getUploadedFileSize($this->data['admitcard']->right_logo,'uploads/admit_card');
                ...
                $insert_data['right_logo'] = $this->data['admitcard']->right_logo;
            }
            ...
            if(isset($_FILES["sign"]) && $_FILES['sign']['name'] != '' && (!empty($_FILES['sign']['name']))) {
                ...
                $prev_file_size += $this->media_storage->getUploadedFileSize($this->data['admitcard']->sign,'uploads/admit_card');
                ...
                $insert_data['sign'] = $this->data['admitcard']->sign;
            }
            ...
            if(isset($_FILES["background_img"]) && $_FILES['background_img']['name'] != '' && (!empty($_FILES['background_img']['name']))) {
                ...
                $prev_file_size += $this->media_storage->getUploadedFileSize($this->data['admitcard']->background_img,'uploads/admit_card');
                ...
                $insert_data['background_img'] = $this->data['admitcard']->background_img;
            }
    ...
    public function delete($id)
    {
        ...
        if ($row->left_logo != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row->left_logo, 'uploads/admit_card');
        ...
        if ($row->right_logo != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row->right_logo, 'uploads/admit_card');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);     
            $this->media_storage->filedelete($row->right_logo, "uploads/admit_card/");
        }

        if ($row->sign != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row->sign, 'uploads/admit_card');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);     
            $this->media_storage->filedelete($row->sign, "uploads/admit_card/");
        }

        if ($row->background_img != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row->background_img, 'uploads/admit_card');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);     

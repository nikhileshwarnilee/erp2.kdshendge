...
class studentidcard extends Admin_Controller
{
        ...
        public function edit($id)
    {
        ...
        try {
            ...
            if ($removebackground_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editidcard[0]->background , 'uploads/student_id_card/background');
            ...
            if ($removelogo_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editidcard[0]->logo , 'uploads/student_id_card/logo');
				$this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);         
            ...
            if ($removesign_image != '') {
				...
				$delete_file_size = $this->media_storage->getUploadedFileSize($editidcard[0]->sign_image , 'uploads/student_id_card/signature');
				$this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);         
            ...
            if (isset($_FILES["background_image"]) && $_FILES['background_image']['name'] != '' && (!empty($_FILES['background_image']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editidcard[0]->background, 'uploads/student_id_card/background');
            ...
            if (isset($_FILES["logo_img"]) && $_FILES['logo_img']['name'] != '' && (!empty($_FILES['logo_img']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editidcard[0]->logo, 'uploads/student_id_card/logo');
            ...
            if (isset($_FILES["sign_image"]) && $_FILES['sign_image']['name'] != '' && (!empty($_FILES['sign_image']['name']))) {
                $prev_file_size += $this->media_storage->getUploadedFileSize($editidcard[0]->sign_image, 'uploads/student_id_card/signature');
    ...
    public function delete($id)
    {
        ...
        if ($row[0]->background != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->background, 'uploads/student_id_card/background');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);          
            $this->media_storage->filedelete($row[0]->background, "uploads/student_id_card/background/");
        }

        if ($row[0]->logo != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->logo, 'uploads/student_id_card/logo');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);          
            $this->media_storage->filedelete($row[0]->logo, "uploads/student_id_card/logo/");
        }

        if ($row[0]->sign_image != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($row[0]->sign_image, 'uploads/student_id_card/signature');
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);          

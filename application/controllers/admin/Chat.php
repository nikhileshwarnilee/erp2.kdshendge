...
class Chat extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data['delete_chat_enable'] = $sch_setting->staff_delete_chat;
    ...
    public function adduser()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (is_null($new_user->image) || $new_user->image == "") {
                if ($new_user->gender == 'Female') {
                    $new_user->image = "uploads/staff_images/default_female.jpg";
                } elseif ($new_user->gender == 'Male') {
                    $new_user->image = "uploads/staff_images/default_male.jpg";
                }
            } else {
                if ($new_user->user_type == "student") {
                    $new_user->image = $new_user->image;
                } else if ($new_user->user_type == "staff") {
                    $new_user->image = "./uploads/staff_images/$new_user->image?" . time();
                }
            }
    ...
    {	
		...
		if (!$userid) {        
			echo json_encode(['status' => 0, 'error' => 'Not logged in', 'count' => 0]);
			exit;
		}
		
        $result    = $this->chatuser_model->get_chat_msg_count($userid);
    ...
    public function get_student_parent_chat_msg_count()
    {
        $userdata = $this->customlib->getUserData();
        $userid   = $userdata['id'];
        $result    = $this->chatuser_model->get_student_parent_chat_msg_count($userid);
        $array = array('status' => '1', 'error' => '', 'count' => count($result));
        echo json_encode($array);
    }
}

...
class Content extends Admin_Controller
{
    ...
    public function ajaxupload()
    {
        ...
        if (isset($_FILES['upload']) && $url == "") {
            ...
            $video_thumb = $this->media_storage->getYoutubeThumbnailSize($this->input->post('url'));
            ...
            if ($video_thumb === false) {
                $this->form_validation->set_rules(
                    ...
                    'validate_video_thumb',
                    'required',
                    array('required' => 'Invalid video URL or not a YouTube video')
                );
            } else {
                $this->form_validation->set_rules(
                    'validate_video_thumb',
                    'validate_video_thumb',
                    "callback_validateCanAddNewResource[storage,{$video_thumb['size_kb']}]"
                );
            }
        }
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
                ...
                'validate_video_thumb'          => form_error('validate_video_thumb'),
            );
            ...
            if (isset($_FILES['upload']) && !empty($_FILES['upload']) && $_FILES['upload']['error'][0] == UPLOAD_ERR_OK) {
                ...
                if ($responses && !empty($responses)) {
                    ...
                    if ($validation == 1) {
                        foreach ($responses['images'] as $key => $value) {
                            $imageFileSize = $this->media_storage->getUploadedFileSize($value['store_name'], 'uploads/school_content/material/media');
                            $this->saasvalidation->updateResouceQuota('storage', $imageFileSize);

                            $thumbnailFileSize = $this->media_storage->getUploadedFileSize($value['thumb_name'], 'uploads/school_content/material/media/thumb');
                            $this->saasvalidation->updateResouceQuota('storage', $thumbnailFileSize);
                ...
                if ($httpcode == 200) {
                    ...
                    if ($upload_response) {
                        ...
                        $imageFileSize = $this->media_storage->getUploadedFileSize($upload_response->store_name, 'uploads/school_content/material/media');
                        $this->saasvalidation->updateResouceQuota('storage', $imageFileSize);

                        $thumbnailFileSize = $this->media_storage->getUploadedFileSize($upload_response->thumb_name, 'uploads/school_content/material/media/thumb');
                        $this->saasvalidation->updateResouceQuota('storage', $thumbnailFileSize);
                ...
                echo json_encode(array('status' => 0, 'error' => array('file' => $e->getMessage())));
            }
        }
    }
    ...
    public function handle_upload_file($field, $var)
    {
        ...
        // Check file exists
        if (!isset($_FILES[$var]) || empty($_FILES[$var]['name'][0])) {
        ...
        $tmp_name   = $_FILES[$var]['tmp_name'][0];
        $error      = $_FILES[$var]['error'][0];

        if ($error !== UPLOAD_ERR_OK) {

            if ($error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE) {
                $this->form_validation->set_message('handle_upload_file', $this->lang->line('no_space_left_on_disk_please_check_your_disk_space_as_per_plan'));
                return false;
            }

            $this->form_validation->set_message('handle_upload_file', "File upload error: " . $error);
            return false;
        }
        ...
        if ($file_size > $result->file_size) {
            $this->form_validation->set_message(
                'handle_upload_file',
                $this->lang->line('file_size_shoud_be_less_than') . number_format($result->file_size / 1048576, 2) . ' MB'
            );
            return false;
        }


        $allowed_extension = array_map('trim', array_map(
            'strtolower',
    ...
    public function delete()
    {
        ...
        if ($is_removed) {
            ...
            //====================
            $delete_file_size1 = $this->media_storage->getUploadedFileSize($upload_content_data->img_name, "$upload_content_data->dir_path");
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size1);

            $delete_file_size2 = $this->media_storage->getUploadedFileSize($upload_content_data->thumb_name,"$upload_content_data->thumb_path");
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size2);
            //====================
        ...
        if ($is_removed) {
            if (!empty($removed_data)) {
                foreach ($removed_data as $remove_data_key => $remove_data_value) {
                    ...
                    //====================
                    $delete_file_size1 = $this->media_storage->getUploadedFileSize($remove_data_value->img_name, "$remove_data_value->dir_path");
                    $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size1);

                    $delete_file_size2 = $this->media_storage->getUploadedFileSize($remove_data_value->thumb_name,"$remove_data_value->thumb_path");
                    $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size2);
                    //====================
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($value->send_to == "public") {
                    ...
                    $share_link = "<button type='button' class='btn btn-primary btn-xs' data-recordid=" . $value->id . " data-link=" . $shared_url_link . " data-toggle='modal' data-target='#linkModal' title=" . $this->lang->line('link') . " ><i class='fa fa-link'></i></button>";
                }

                $editbtn = "<button type='button' class='btn btn-primary btn-xs' data-recordid=" . $value->id . " data-toggle='modal' data-target='#viewShareModal' title=" . $this->lang->line('view') . " ><i class='fa fa-eye'></i></button>";

                if ($this->rbac->hasPrivilege('content_share_list', 'can_delete')) {
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' href='" . base_url() . "admin/content/delete_content/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                } else {
                    $deletebtn = "";
                }

...
class Homework extends Admin_Controller
{
        ...
        if (!empty($homework->data)) {
            foreach ($homework->data as $homework_key => $homeworklist) {
                ...
                if ($this->rbac->hasPrivilege('homework_evaluation', 'can_view')) {
                    $viewbtn = "<a onclick='evaluation(" . '"' . $homeworklist->id . '"' . ")' title=''  data-toggle='tooltip'  data-original-title=" . $this->lang->line('evaluation') . " class='btn btn-primary btn-xs'  title='" . $this->lang->line('evaluation') . "' data-toggle='tooltip'><i class='fa fa-reorder'></i></a>";
                }

                if ($this->rbac->hasPrivilege('homework', 'can_edit')) {
                    $editbtn = "<a  class='btn btn-primary btn-xs modal_form'  data-toggle='tooltip'   data-method_call='edit' data-original-title='" . $this->lang->line('edit') . "' data-record_id=" . $homeworklist->id . " ><i class='fa fa-pencil'></i></a>";
                }
        ...
        try {
            ...
            if ($record_id > 0) { //edit
                ...
                if (isset($_FILES["userfile"]) && $_FILES['userfile']['name'] != '' && (!empty($_FILES['userfile']['name']))) {
                    $prev_file_size = $this->media_storage->getUploadedFileSize($homeworklist['document'], 'uploads/homework');
                ...
                if (IsNullOrEmptyString($img_name)) {  // check upload image has not uploaded successfully
                    $total_documents_failed_size += $this->media_storage->getTmpFileSize('userfile'); 
                    // get temp size of image because of image not uploaded 
                }
    ...
    public function delete($id)
    {
        ...
        if (!empty($id)) {
            ...
            if ($row['document'] != '') {
                $delete_file_size = $this->media_storage->getUploadedFileSize($row['document'], 'uploads/homework');
    ...
    public function add_evaluation()
    {
        ...
        if (!empty($students) && (!empty($homeworkresult['marks']))) {
    ...
    public function searchdailyassignment()
    {
        ...
        if (!empty($dailyassignment->data)) {
            ...
            foreach ($dailyassignment->data as $key => $value) {
                ...
                if ($value->attachment != "") {
                    $download = "<a  href='" . base_url() . "homework/dailyassigmnetdownload/" . $value->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip'  title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }

                $assignment = '<a onclick="assignmentdetails(' . $value->id . ')" class="btn btn-primary btn-xs" data-target="#assignmentdetails" data-backdrop="static" data-keyboard="false" data-toggle="modal"  title="' . $this->lang->line('view') . '"><i class="fa fa-reorder"></i></a>';
                ...
                $row[] = $evaluation_date;
                $row[] = $evaluated_by;
                $row[] = $download . ' ' . '<a onclick="assignmentevaluation(' . $value->id . ')" class="btn btn-primary btn-xs" data-target="#assignmentevaluation" data-backdrop="static" data-keyboard="false" data-toggle="modal"  title="' . $this->lang->line('evaluate') . '"><i class="fa fa-newspaper-o"></i></a>' . ' ' . $assignment;
...
if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
    ...
    public function searchdailyassignmentreport()
    {
        ...
        if (!empty($dailyassignment->data)) {
            ...
            foreach ($dailyassignment->data as $key => $value) {
                ...
                $assignment = '<a onclick="dailyassignmentdetails(' . $value->student_id . ')" class="btn btn-primary btn-xs" data-target="#dailyassignmentdetails" data-backdrop="static" data-keyboard="false" data-toggle="modal"  title="' . $this->lang->line('view') . '"><i class="fa fa-reorder"></i></a>';
    ...
    public function homework_marksreport()
    {        

        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'homework/homework_marksreport');
        $this->session->set_userdata('subsub_menu', 'homework/homework_marksreport');
        $data['sch_setting']      = $this->setting_model->getSetting();
        $class                    = $this->class_model->get();
        $data['classlist']        = $class;
        $userdata                 = $this->customlib->getUserData();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['resultlist'] = $this->homework_model->search_homework_marks_report($class_id, $section_id, $subject_group_id, $subject_id);        
        }

        $this->load->view("layout/header");
        $this->load->view("homework/homework_marksreport", $data); 
        $this->load->view("layout/footer");
    }
}

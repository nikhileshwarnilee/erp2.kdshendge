...
if (empty($syllabus_data)) {
            ...
            foreach ($syllabus_data as $tm_k => $tm_kue) {
                ?>
                                            <div class="mright5 text-right float-right mt5">
                                                <a class="btn btn-primary btn-xs pull-left" data-toggle="tooltip" onclick="get_subject_syllabus('<?php echo $tm_kue['id']; ?>')" title="" data-original-title="<?php echo $this->lang->line('view'); ?>" ><i class="fa fa-reorder"></i></a>
                                            </div>

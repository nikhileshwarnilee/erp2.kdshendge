...
$(document).on('input','.chat_input',function(){
    ...
    $(document).on('click', '.contact', function () {
        get_student_parent_chat_msg_count();//function declared in footer
    ...
    function newChatMessage() {
        ...
        var delete_chat_enable=$("#delete_chat_enable").val();
        var display_status;
        
        if(delete_chat_enable==0){
            display_status="style='display: none;'";
        }else{
            display_status="style='display: visible;'";
        }
        ...
        if (chat_connection_id > 0 && chat_to_user > 0) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('<li class="replies chat_msg" data-msg_id="'+data.last_insert_id+'"  id="reply_'+data.last_insert_id+'"><p><i '+ display_status +' class="fa fa-remove text-danger point remove_btn"  data-toggle="tooltip" data-original-title="<?php echo $this->lang->line('delete') ?>"  onclick="delete_msg('+data.last_insert_id+')"></i>' + message + '</p> <span class="time_date_send"> ' + date_time_temp + '</span></li>').appendTo($('.messages ul'));
                    $('.chat_input').val(null);
    ...
    // delete msg
    function delete_msg_old(msg_id){
        let text = "<?php echo $this->lang->line('are_you_sure'); ?>";
        if (confirm(text) == true) {
        $.ajax({
            type: "POST",
            url: baseurl + 'user/chat/delete_msg',
            data: {msg_id:msg_id},
            success: function (data) {
                console.log(data);
            },
    ...
    function get_active_chat_msg(){
        ...
        $.ajax({
            type: "POST",
            url: baseurl + 'user/chat/get_active_chat_msg',
            data: {'chat_connection_id': chat_connection_id},
            dataType: "JSON",
            success: function (data) {
...
// delete msg
    ...
    function mynewUser() {
        //sidebar chat list set time out 

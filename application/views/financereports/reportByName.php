...
echo $this->lang->line('fees_statement') . " ";
            ...
            ?></div>
                                    <table class="table table-striped table-bordered table-hover example table-fixed-header"  data-export-title="<?php echo $this->lang->line('fees_statement');?>">
                                        <thead class="header">
                                            <tr>
...
if (!empty($student['transport_fees'])) {
    ...
    foreach ($student['transport_fees'] as $transport_fee_key => $transport_fee_value) {
        ...
        if (!empty($transport_fee_value->amount_detail)) {
            ...
            foreach ($fee_deposits as $fee_deposits_key => $fee_deposits_value) {
                $fee_paid     = (int) $fee_paid + (int) $fee_deposits_value->amount;
                $fee_discount = (int) $fee_discount + (int) $fee_deposits_value->amount_discount;
                $fee_fine     = (int) $fee_fine + (int) $fee_deposits_value->amount_fine;
            }
        }

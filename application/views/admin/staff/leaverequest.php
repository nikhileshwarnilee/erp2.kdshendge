                    ...
                    <div class="box-header ptbnull">
                        ...
                        <div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('apply_leave', 'can_add')) {?>
                                <a href="#addleave" onclick="addLeave()" role="button" class="btn btn-primary checkbox-toggle pull-right edit_setting" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> <?php echo $this->lang->line('processing'); ?>"><?php echo $this->lang->line('apply_leave'); ?></a>
                            <?php }?>
                        </div>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="tab-pane active table-responsive no-padding">
                                    ...
                                    <table class="table table-striped table-bordered table-hover example" data-export-title="<?php echo $this->lang->line('leaves');?>">
                                        <thead>
...
foreach ($leave_request as $key => $value) {
    ?>
                                                <tr>
                                                    ...
                                                    <td><span data-toggle="popover" class="detail_popover" data-original-title="" title=""><?php echo $value['name'] . " " . $value['surname']." (".$value['employee_id'].")"; ?></span>
                                                        <div class="fee_detail_popover" style="display: none">
                                                            <p class="text text-info">
                                                                <?php echo $this->lang->line('staff_id'); ?>: <?php echo $value['employee_id']; ?>
                                                            </p>
                                                        </div>
                                                    </td>
...
?>
                                                    ...
                                                    <td class="pull-right no-print"><a href="#leavedetails" onclick="getRecord('<?php echo $value["id"] ?>')" role="button" class="btn btn-primary btn-xs" data-toggle="tooltip" title="<?php echo $this->lang->line('view'); ?>" ><i class="fa fa-reorder"></i></a>
                                                        <?php if ($can_delete == 1) {?>
                                                            <a onclick="getDelete('<?php echo $value["id"] ?>','<?php echo $value["staff_id"] ?>')"  class="btn btn-primary btn-xs" data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" ><i class="fa fa-remove"> </i></a>
                                                        <?php }?>
                                                    </td>
                                                </tr>
                                                <?php
...
?>
						...
						<div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12 hideshowdiv">
                                          
                            <label class="radio-inline">
                                <input type="radio" id="half_day_am" name="half_day_leave" value="half_day" >&nbsp;&nbsp;<?php echo $this->lang->line('half_day_first_half'); ?>
                            </label>
                            <label class="radio-inline">
                                <input type="radio" id="half_day_pm" name="half_day_leave" value="half_day_second_shift" >&nbsp;&nbsp;<?php echo $this->lang->line('half_day_second_shift'); ?>
                            </label>
...
$('#leave_from_date, #leave_to_date').on('changeDate change', function () {
    calculateDays();
});
    ...
    function calculateDays() {        
        ...
        let  fromDate = $('#leave_from_date').val();
        let  toDate = $('#leave_to_date').val();      

        if (fromDate && toDate) {        
       
            $.ajax({
                url: base_url + 'admin/leaverequest/getDateDifference',
                type: 'POST',
                data: {date_1: fromDate, date_2:toDate},          
                success: function (result) {
                
                    if (result > 1) {
                        $('#half_day_am, #half_day_pm').prop('disabled', true).prop('checked', false);
                        $('.hideshowdiv').addClass('hide');
                    } else {
                        $('#half_day_am, #half_day_pm').prop('disabled', false);
                        $('.hideshowdiv').removeClass('hide');
                    }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.document_file != "") {
                    ...
                    $("#download_file").html('<a href=' + base_url + 'admin/leaverequest/downloadleaverequestdoc/' + result.staff_id + '/' + result.id + ' class=btn-xs  data-toggle=tooltip data-original-title="<?php echo $this->lang->line("download"); ?>">' + cl + '</a>');
                }
            }
        });

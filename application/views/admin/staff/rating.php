...
<div class="content-wrapper">
            ...
            <div class="col-md-12">
                <div class="box box-info" id="attendencelist">
                    ...
                    <div class="box-body table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover example xyz" data-export-title="<?php echo $this->lang->line('teachers_rating_list');?>">
                            <thead>
                                <tr>
    ...
    ?>
                                    <tr>
                                        <td class="dt-body-left dt-head-left"><?php echo $value['employee_id']; ?></td>
                                        <td><a href="<?php echo base_url(); ?>admin/staff/profile/<?php echo $value['id']; ?>" ><?php echo $value['name']; ?></a></td>
                                        <td><span class="hide-on-print"><?php
$j = 5;
    for ($i = 1; $i <= $j; $i++) {
        ?>
                                            ...
                                            <?php }
    ;?> </span>
                                            <span class="print-only"  > <?php echo $value['rate']; ?></span>
...
if ($this->rbac->hasPrivilege('teachers_rating', 'can_edit')) {
        ...
        if ($value['status'] == '0') {
            ?><a style="vertical-align: middle"  class="label label-info btn btn-xs " href="<?php echo base_url(); ?>admin/Staff/ratingapr/<?php echo $value['rate_id'] ?>"><?php echo $this->lang->line('approve'); ?></a><?php } else {?><?php
}
    }
    if ($this->rbac->hasPrivilege('teachers_rating', 'can_delete')) {
        ?>
                                                <a class="btn btn-primary btn-xs" data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>"  onclick="return confirm('<?php echo $this->lang->line('delete_confirm'); ?>');" href="<?php echo base_url(); ?>admin/Staff/delete_rateing/<?php echo $value['rate_id'] ?>"><i class="fa fa-remove"></i></a>
                                                    <?php
}
    ?>
                                        </td>

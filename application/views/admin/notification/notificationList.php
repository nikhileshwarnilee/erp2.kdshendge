                ...
                <div class="box box-solid1 box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools pull-right">
                            ...
                            <?php if ($this->rbac->hasPrivilege('notice_board', 'can_delete')) {?>
                                <a class="btn btn-primary btn-sm pull-right checkbox-toggle delete_log"> <?php echo $this->lang->line('delete_notice_board'); ?></a>
                            <?php }?>
                              
                            <?php if ($this->rbac->hasPrivilege('notice_board', 'can_add')) {?>
                                <a href="<?php echo base_url() ?>admin/notification/add" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> <?php echo $this->lang->line('post_new_message'); ?></a> &nbsp
                            <?php }?>
...
if (empty($notificationlist)) {
    ?>
                        <br><div class="alert alert-info"><?php echo $this->lang->line('no_record_found'); ?></div>
                    <?php
} else {
    ...
    $(function () {
        $('.delete_log').on('click', function () {            
            if (confirm("<?php echo $this->lang->line('confirm_delete') ?>")) {             
                $.ajax({
                    url: '<?php echo base_url(); ?>admin/notification/delete_notice_board_log/',

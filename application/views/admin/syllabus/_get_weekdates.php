...
$day_counter = 0;
    foreach ($timetable as $tm_key => $tm_value) {
        ...
        $day_counter++;
        ?>
                        <th class="text v-align-top"><?php echo $this->lang->line(strtolower($tm_key)); ?><br/><span class="bmedium"><?php echo date($this->customlib->getSchoolDateFormat(), strtotime($next_date)); ?></span></th>
                    <?php }
    ?>
                </tr>
            </thead>
...
$day_counter = 0;
    foreach ($timetable as $tm_key => $tm_value) {
        ...
        $day_counter++;
        ?>
                        <td class="text v-align-top" width="14%">
            ...
            foreach ($timetable[$tm_key] as $tm_k => $tm_kue) {
                ...
                if ($action != 0) {
                    ?>
                                        <div class="d-flex gap-0-5 mright5 text-right float-right" id="hide_<?php echo $action; ?>">
                                            <a class="btn btn-primary btn-xs mt5 mb5 pull-left"  data-toggle="tooltip" title="" data-original-title="<?php echo $this->lang->line('view') ?>" onclick="get_subject_syllabus(<?php echo $action; ?>)"><i class="fa fa-reorder"></i></a>

                                            <?php if ($this->rbac->hasPrivilege('manage_lesson_plan', 'can_edit')) {?>
                                                <a class="btn btn-primary btn-xs mt5 mb5 pull-left"  data-toggle="tooltip" title="" data-original-title="<?php echo $this->lang->line('edit') ?>" onclick="subject_syllabusedit(<?php echo $action; ?>)"><i class="fa fa-pencil"></i></a>
                                                <a class="btn btn-primary btn-xs mt5 mb5 pull-left"  data-toggle="tooltip" title="" data-original-title="<?php echo $this->lang->line('delete') ?>" onclick="subject_syllabusdelete('<?php echo $action; ?>', '<?php echo $tm_kue->subject_group_subject_id; ?>', '<?php echo $tm_kue->time_from ?>', '<?php echo $tm_kue->time_to; ?>', '<?php echo $this->customlib->dateformat($new_date); ?>', '<?php echo $subject_group_subject_class_section['id']; ?>')"><i class="fa fa-remove"></i></a>
                                                <?php
}
                    ?>
                                        </div>
                    ...
                    ?>
                                        ...
                                        <div class="d-flex gap-0-5 mright5 text-right float-right">

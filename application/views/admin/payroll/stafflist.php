...
if (isset($resultlist)) {
    ?>
                        ...
                        <div class="box-body table-responsive">
                            ...
                            <table class="table table-striped table-bordered table-hover example" data-export-title="<?php echo $this->lang->line('staff_list');?>">
                                <thead>
                                    <tr>
        ...
        ?>
                                        <tr>
                                            ...
                                            <?php }if ($sch_setting->staff_phone) {?>
                                                ...
                                                <?php }?>
                                            <td><small <?php echo $label; ?>><?php if($year > 0){ echo $wstatus; } ?></small></td>
                                                <?php if ($status == "paid") {
            ?>
...
if ($this->rbac->hasPrivilege('staff_payroll', 'can_delete')) {
                ?>
                                                        <a class="btn btn-primary btn-xs" onclick="return confirm('<?php echo $this->lang->line("are_you_sure_you_want_to_revert_this_record") ?>')" href="<?php echo base_url() . "admin/payroll/revertpayroll/" . $staff["payslip_id"] . "/" . $month_selected . "/" . date("Y") . "/" . $role_selected ?>" title="<?php echo $this->lang->line("revert") ?>">
            ...
            ?>
                                                <td class="pull-right no-print white-space-nowrap">
                                                    ...
                                                    <a href="<?php echo site_url('admin/payroll/edit/' . $staff["payslip_id"]) ?>" class="btn btn-primary btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('edit') ?>"><i class="fa fa-pencil"> </i></a>
                                                    <?php }?>
            <?php
if ($this->rbac->hasPrivilege('staff_payroll', 'can_delete')) {
                ?>
                                                        <a href="<?php echo base_url() ?>admin/payroll/deletepayroll/<?php echo $staff["payslip_id"] . "/" . $month_selected . "/" . date("Y") . "/" . $role_selected ?>" class="btn btn-primary btn-xs" onclick="return confirm('<?php echo $this->lang->line("are_you_sure_you_want_to_revert_this_record") ?>')" title="<?php echo $this->lang->line("revert") ?>">
                                                            <i class="fa fa-undo"> </i>
                                                        </a>
                <?php
}
...
<div id="payslipview"  class="modal fade" role="dialog">
    <div class="modal-dialog modal-dialog2 modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body pb30" id="testdata">
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
 ...
 function getPayslip(id) {
        ...
        $.ajax({
            ...
            data: {payslipid: id},
            success: function (result) {
                $("#print1").html("<a href='#'  class='pull-right modal-title moprintblack mt1 mright8'  onclick='printData(" + id + ")'  title='Print' ><i class='fa fa-print font14'></i></a>");

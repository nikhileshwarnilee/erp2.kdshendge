    ...
    <section class="content" style="position: relative;">
        <div class="row">
            <div class="col-md-12">
                ...
                <div id="frame">
                    <div id="sidepanel">
                        <input type="hidden" name="delete_chat_enable" id="delete_chat_enable" value="<?php echo $delete_chat_enable; ?>">
        ...
        <form id="addUser" action="<?php echo site_url('admin/chat/adduser') ?>" method="POST">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait'); ?>"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add'); ?></button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    $(document).on('click', '.contact', function() {
        get_chat_msg_count(); //function declared in footer
...
function newChatMessage() {
    ...
    const message = htmlEncode($input.val().trim());
    ...
    const chat_connection_id = $("input[name='chat_connection_id']").val();
    const chat_to_user = $("input[name='chat_to_user']").val();
    const delete_chat_enable = $("#delete_chat_enable").val();

    const display_status = delete_chat_enable == 0
        ? "style='display: none;'"
        : "style='display: inline;'"; // use valid CSS display value
    ...
    if (chat_connection_id > 0 && chat_to_user > 0) {
        $.ajax({
            ...
            success: function(data) {
                ...
                const $chatList = $('.messages ul');
                const deleteBtn = `
                    <i ${display_status} class="fa fa-remove text-danger point remove_btn" 
                        data-toggle="tooltip" 
                        data-original-title="<?php echo $this->lang->line('delete') ?>"  
                        onclick="delete_msg(${data.last_insert_id})">
                    </i>`;

                const newMessageHtml = `
                    <li class="replies chat_msg" data-msg_id="${data.last_insert_id}" id="reply_${data.last_insert_id}">
                        <p>${deleteBtn}${message}</p>
                        <span class="time_date_send">${date_time_temp}</span>
                    </li>`;
                ...
                $('.contact.active .preview').html(
                    `<span><?php echo $this->lang->line('you'); ?>: </span>${message}`
                );
    ...
    function delete_msg(msg_id) {
        let count = $('.remove_btn').length;
        // Find the delete icon for this message
        let indexno = $("#reply_" + msg_id).find('.remove_btn');
        let classes = $("#reply_" + msg_id).attr('class');
        // Get its index among all .remove_btn elements
        let index = $('.remove_btn').index(indexno);

        let reply_msg_id = $("#reply_" + msg_id).data("msg_id");
        let sent_msg_id = $('.sent.chat_msg').last().data('msg_id');

        if (((parseInt(count) - parseInt(1)) == parseInt(index)) && (parseInt(reply_msg_id) >= parseInt(sent_msg_id))) {
            $(".contact.active").find(".preview").html("");
        }
        let text = "<?php echo $this->lang->line('are_you_sure'); ?>";
        if (confirm(text) == true) {
            $.ajax({
                type: "POST",
                url: base_url + 'admin/chat/delete_msg',
                data: {
                    msg_id: msg_id
                },
                success: function(data) {
    ...
    function get_active_chat_msg() {
        ...
        $.ajax({
            type: "POST",
            url: base_url + 'admin/chat/get_active_chat_msg',
            data: {
                'chat_connection_id': chat_connection_id
            },
            dataType: "JSON",
            success: function(data) {
                ...
                $(".chat_msg").each(function(index) {
                    ...
                    var plainArray = [];
                    var idArray = data.chatList.map(function(chat) {
                        return chat.id; // Extract the 'id' property from each object
                    });

                    if (jQuery.inArray(String(msg_id), idArray) != -1) {

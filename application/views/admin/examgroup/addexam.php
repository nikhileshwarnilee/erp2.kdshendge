...
if ($this->rbac->hasPrivilege('exam', 'can_add')) {
    ?>
                                <a tabindex="-1" class="btn btn-primary" href="#" id="examModalButton"> <?php echo $this->lang->line('new_exam'); ?></a>
                                <?php
}
if ($this->rbac->hasPrivilege('link_exam', 'can_view')) {
    ?>
                                <a tabindex="-1" class="btn btn-primary"  data-toggle="modal" data-target="#examconnectModal" href="#" id="examconnectModalButton" data-examGroup_id="<?php echo $examgroup->id; ?>"> <?php echo $this->lang->line('link_exams'); ?></a>
                                <?php
}
?>
                        </div>
...
?>
                        ...
                        <div class="col-sm-12 col-lg-12 col-md-12">
                            ...
                            <span class="text text-danger" id="marksheet_error"></span>
                                </div>
                            </div>
...
<script>
    $(document).on('click', '.add', function () {
        ...
        html += '<td class="text-center" style="vertical-align: middle; cursor: pointer;"><span class="btn btn-primary remove fa fa-times mt5"></span></td></tr>';
...
<script type="text/javascript">
    ...
    if (res.status == "0") {
        ...
        '<"dt-layout-cell dt-layout-end text-end"l>' +
        '>' +
        'rt' +
        ...
        '>',
                            "lengthChange": true,
                             "columnDefs": [
  {
    targets: [-1, 6, 7], // last column, column 6, and column 7
    orderable: false,
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // $.each(arr, function (index, value) {
                        ...
                        // let adm_no_csv = $.trim(value.adm_no);
						
						
                        // $("#std_exam_marks_"+adm_no_csv).val(value.marks);
                        // $("#std_exam_note_"+adm_no_csv).val(value.note);
                        // if(value.attendence == 0){
                            // $("#std_exam_attendance_"+adm_no_csv).prop( "checked", true );
                        // }else{
                            // $("#std_exam_attendance_"+adm_no_csv).prop( "checked", false);
                        // }
                    // });
					...
					$.each(arr, function (index, value) {
						let adm_no_csv = (value.adm_no || '').toString().trim().normalize('NFC');
						let note = value.note || '';
						let marks = value.marks || '';
						let isAbsent = parseInt(value.status) === 0;
					
						let $match = $('tr[data-adm_no]').filter(function () {
							let domAdmNo = ($(this).data('adm_no') || '').toString().trim().normalize('NFC');
							return domAdmNo === adm_no_csv;
						});
					
						if ($match.length) {
							 
							$match.find('td input.note').val(note);							
		
							if (value.attendence == 0) {                                
								$match.find('td input.attendance_chk').prop("checked", true);
								$match.find('td input.marksssss').val(0);
                            } else {                                
								$match.find('td input.attendance_chk').prop("checked", false);
								$match.find('td input.marksssss').val(marks);
                            }						

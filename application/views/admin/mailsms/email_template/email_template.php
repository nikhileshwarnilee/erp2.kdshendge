                ...
                <div class="box box-info">
                    <div class="box-header ptbnull">
                        ...
                        <?php if ($this->rbac->hasPrivilege('email_template', 'can_add')) {?>
                            <button type="button" class="btn btn-primary" onclick="add()"  data-backdrop="static" ><i class="fa fa-plus"></i> <?php echo $this->lang->line('add'); ?></button>
                        <?php }?>
                        </div>
                    </div>
                    ...
                    <div class="box-body">
                        <div class="table-responsive overflow-visible">
                            ...
                            <table class="table table-hover table-striped table-bordered example "  data-export-title="<?php echo $this->lang->line('email_template_list');?>">
                                <thead>
                                    <tr>
...
foreach ($email_template_list as $key => $email_template_list_value) {?>
                                        <tr>
                                           ...
                                           <button type="button" class="btn btn-primary btn-xs"  data-toggle="tooltip"  title="<?php echo $this->lang->line('attachment'); ?>" onclick="viewdocuments(<?php echo $email_template_list_value['id'] ?>)" ><i class="fa fa-download"></i></button>



                                                <?php if ($this->rbac->hasPrivilege('email_template', 'can_edit')) {?>
                                                <a class="btn btn-primary btn-xs editemailtemplate" data-id="<?php echo $email_template_list_value['id'] ?>"  data-toggle="tooltip" title="<?php echo $this->lang->line('edit'); ?>"><i class="fa fa-pencil"></i>
                                                </a>
                                                <?php }if ($this->rbac->hasPrivilege('email_template', 'can_delete')) {?>
                                                <a href="<?php echo base_url(); ?>admin/mailsms/delete_email_template/<?php echo $email_template_list_value['id'] ?>"class="btn btn-primary btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="return confirm('<?php echo $this->lang->line('delete_confirm') ?>');"><i class="fa fa-remove"></i>
                                                </a>
                                                <?php }?>
                                            </td>
                                        </tr>
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                             ...
                             <div class="row">
                                <div class="col-sm-12">
                            <div class="form-group">
                            <div id="file_attachment"  class=""></div>
                            <div id="my_attachment" class=""></div>
                            </div>
                            </div>
                            </div>
                            <div class="row">
...
<!-- ======================================= -->
<div class="modal fade" id="viewdocuments" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="box-title"><?php echo $this->lang->line('attachment'); ?></h4>
            </div>
            <div class="modal-body ptt10 pb10" id="setdocuments">
            </div>
        </div>
    </div>
</div>
...
<!-- ======================================= -->
<script>
    function viewdocuments(template_id){
         $('#viewdocuments').modal({
            backdrop: 'static',
            keyboard: false
        });
        ...
        $.ajax({
            url:'<?php echo site_url("admin/mailsms/viewdocuments"); ?>',
            type:'post',
            data:{template_id:template_id},
            dataType:'json',
            success:function(response){
                 $('#setdocuments').html(response.page);
            }
        });
    }
</script>

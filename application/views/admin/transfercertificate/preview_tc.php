<style>
    .table-bordered-left>tbody>tr>td, 
    .table-bordered-left>tbody>tr>th, 
    .table-bordered-left>tfoot>tr>td, 
    .table-bordered-left>tfoot>tr>th, 
    .table-bordered-left>thead>tr>td, 
    .table-bordered-left>thead>tr>th {
...
?>
<div style="width: 99%; margin: 10px auto;border: solid 1px var(--bs-table-border); font-size: 16px;">
 <div class="table-responsive">   
<table class="table mb0">
    <tbody>
 <tr>
 <td>
    <img src="<?php echo $this->media_storage->getImageURL('/uploads/transfer_certificate/'.$get_settings[0]['header_image']); ?>" style="height: auto;width: 100%;">
 </td>
</tr>
<tr>
<td style="margin:10px;padding: 10px;">
    <h2 class="text-center"><?php echo $this->lang->line('transfer_certificate');?><br> <?php if($is_regenerte==1){ echo ('<small>['. $this->lang->line('reissue') .']</small>');} ?></h2>
</td>
</tr>
...
<tr>
<td>
   <table class="table table-bordered-left mb0">
    
    
<?php
$sch_setting_array = json_decode(json_encode($sch_setting), true);
$sl = 0;    
...
foreach($getallfields as $key=>$value){  
    if ($this->customlib->checkfieldexist_transfer_certificate($value['name']) && $value['is_default']==1 && $value['name'] != 'tc_no') { 
        ...
        ?>
        <tr>
            <td class="text-center" width="5%"><?php echo $sl; ?></td> 
            <td><?php echo $this->lang->line($value['lang_key']); ?></td>
            <td><?php 
                if($value['name']=="firstname"){
                    echo $student_name = $this->customlib->getFullName($student['firstname'],$student['middlename'],$student['lastname'],$sch_setting->middlename,$sch_setting->lastname);
                }else if($value['name']=="admission_no"){
...
<!-- signature section -->
<table class="table table-bordered-left text-center">
    <tr>
         <?php
        foreach($get_settings as $skey=>$svalue){
            if($svalue['class_teacher_signature']!=""){ ?>
                <td style="padding:32px 20px 20px;text-align: center;border-left:0; border-right:0">
					...
					<br>
					<?php echo $this->lang->line('class_teacher_signature');?>
                </td>
            <?php } ?>
            <?php if($svalue['checked_by']!=""){ ?>
                <td style="padding:32px 20px 20px;text-align: center;">
					...
					<br>
					<?php echo $this->lang->line('checked_by');?>
                </td>
            <?php } ?>
            <?php if($svalue['signature_of_principle']!=""){ ?>
                <td style="padding:32px 25px 20px;text-align: center;border-left:0; border-right:0">
					...
					<br>
					<?php echo $this->lang->line('principle_signature');?>
                </td>
            <?php } ?>
        <?php
        }
...
<!-- signature section -->
<div style="width: 100%; margin: 0 auto; padding: 0 5px;">
<b><?php echo $get_settings[0]['footer_content']; ?></b>
</div>
</div>
</div>
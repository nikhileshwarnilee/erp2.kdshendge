...
<tr>
    ...
    <td class="mailbox-name"> <?php echo $language['language'] ?> <span
            class="flag-icon flag-icon-<?php echo $language['country_code']; ?>"></span></td>
    <td><?php echo $language['short_code']; ?></td>

    <td><input type="text" data-id="<?php echo $language['id']; ?>" id="country_code<?php echo $language['id']; ?>"
            class="form-control country_code" value="<?php echo $language['country_code']; ?>"></td>
...
if ($lang['lang_id'] == $language['id']) {
    ...
    <td>
        <input type="checkbox" id="rtl_<?php echo $language["id"]; ?>" value="<?php echo $language["id"]; ?>"
            name="defoult" onchange="rtl(this.value)" <?php if ($language['is_rtl'] == '1') {echo "checked";}?>>
    </td>
    <td class="relative text-right">
        ...
        <a onclick="return confirm('<?php echo $this->lang->line('delete_confirm') ?>');"
            href="<?php echo base_url(); ?>admin/language/delete/<?php echo $language['id']; ?>"
            class="btn btn-primary btn-xs mr-1" data-toggle="tooltip" title=""
    ...
    if ($this->customlib->getSessionLanguage() != $language['id']) {
        ...
        <div class="material-switch pull-right">
            ...
            <input id="student<?php echo $language["id"] ?>" name="someSwitchOption001" type="checkbox" <?php
if (in_array($language["id"], $selected_lang)) {
                echo "data-role='1'";
            } else {
                echo "data-role='2'";
            ...
            ?> />
            <label for="student<?php echo $language["id"] ?>" class="label-info-success"></label>
        </div>
        <?php }
    }
    ?>

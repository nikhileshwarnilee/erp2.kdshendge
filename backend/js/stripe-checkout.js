...
async function initialize(event) {
    ...
    const { paymentIntentId, clientSecret } = await fetch(baseurl + "user/gateway/stripe/create_payment_intent", {
        method: "POST",
...
async function handlePaymentSubmit(e) {
    ...
    const { customer_id } = await fetch(baseurl + "user/gateway/stripe/create_customer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (paymentIntent) {
        switch (paymentIntent.status) {
            case "succeeded":
                ...
                fetch(baseurl + "user/gateway/stripe/insert_payment", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },

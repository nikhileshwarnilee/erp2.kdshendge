...
<div class="box-header ptbnull">
    ...
    ?></div>
                            ...
                            <a class="btn btn-default btn-xs pull-right" id="btnExport" onclick="return fnExcelReport();"> <i class="fa fa-file-excel-o"></i> </a>

                            <table class="table table-striped table-bordered table-hover " id="headerTable">
                                <thead class="header">
                                    <tr>
                                        <th style="mso-number-format:'\@';"><?php echo $this->lang->line('payment_id'); ?></th>
    ...
    foreach ($results as $key => $value) {
        ...
        ?>
                ...
                <?php 
                 foreach ($value as $collect) { ?>
                        <tr>
                            <td class="excel-text" style="mso-number-format:'\@';white-space:nowrap;"><?php echo $collect['id'] . "/" . $collect['inv_no']; ?></td>                
                            ...
                            <td class="text text-right">
                                <?php echo number_format(($collect['amount'] + $collect['amount_fine']), 2, '.', ''); ?>
                            </td>                               
...
function fnExcelReport() {
    ...
    var excelTextCells = exportTable.querySelectorAll('.excel-text');

    for (var i = 0; i < excelTextCells.length; i++) {
        var cell = excelTextCells[i];
        var textValue = cell.textContent || cell.innerText || '';
        var existingStyle = cell.getAttribute('style') || '';
        cell.textContent = textValue;
        cell.setAttribute('style', existingStyle + ';mso-number-format:"\\@";white-space:nowrap;');
    }

    var tab_text = "<table border='2px'>" + exportTable.innerHTML + "</table>";
    ...
    link.href = fileURL;
    link.download = 'fees_collection_report_<?php echo date('Ymd_His'); ?>.xls';

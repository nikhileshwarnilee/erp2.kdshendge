...
class Feetype_model extends MY_Model
{
    ...
    public function updateSearchDueFeesStatus($id, $status)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->where('is_system', 0);
        $this->db->update('feetype', array('include_in_search_due_fees' => $status));
        $message   = UPDATE_RECORD_CONSTANT . " On fee type search due fees id " . $id;
        $action    = "Update";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
